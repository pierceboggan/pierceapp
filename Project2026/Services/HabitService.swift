...
class HabitService: ObservableObject {
    ...
    var activeHabits: [HabitTemplate] {
        habitTemplates.filter { $0.isActive }
    }
    
    var coreHabits: [HabitTemplate] {
        habitTemplates.filter { $0.isCore }
    }
    
    var customHabits: [HabitTemplate] {
        habitTemplates.filter { !$0.isCore }
    }
    
    func habitsByCategory(_ category: HabitCategory) -> [HabitTemplate] {
        activeHabits.filter { $0.category == category }
    }
    
    func addHabit(_ habit: HabitTemplate) async {
        habitTemplates.append(habit)
        await saveTemplates()
    }
    
    func updateHabit(_ habit: HabitTemplate) async {
    ...
    func toggleHabit(_ habit: HabitTemplate) async {
        if let index = habitTemplates.firstIndex(where: { $0.id == habit.id }) {
    ...
    func archiveHabit(_ habit: HabitTemplate) async {
        if let index = habitTemplates.firstIndex(where: { $0.id == habit.id }) {
    ...
    func deleteHabit(_ habit: HabitTemplate) async {
    ...
    func updateHabitLog(_ habit: HabitTemplate, value: Double?, duration: Int?, note: String?, on date: Date = Date()) async {
        let dayStart = Calendar.current.startOfDay(for: date)
    ...
    func streak(for habit: HabitTemplate) -> Int {
        var streak = 0
    ...
    func weeklyCompletionCount(for habit: HabitTemplate) -> Int {
