...
/// Aggregates daily data from all services into DaySummary records for historical analysis.
/// Calculates weekly summaries, habit compliance streaks, and average scores.
/// Powers the History view calendar and trend visualizations.
@MainActor
public class DaySummaryService: ObservableObject {
    ...
    public func summaryForDate(_ date: Date) -> DaySummary? {
    ...
    public func updateReflection(for date: Date, note: String) async {
        let dayStart = Calendar.current.startOfDay(for: date)
    ...
    public func summariesForWeek(containing date: Date) -> [DaySummary] {
    ...
    public func weeklySummary(for date: Date) -> WeeklySummary {
        let calendar = Calendar.current
    ...
    public func summariesForMonth(year: Int, month: Int) -> [DaySummary] {
        let calendar = Calendar.current
    ...
    public func recentSummaries(count: Int = 30) -> [DaySummary] {
        summaries
            .sorted { $0.date > $1.date }
            .prefix(count)
            .map { $0 }
    ...
    public func averageScore(lastDays: Int = 7) -> Double {
